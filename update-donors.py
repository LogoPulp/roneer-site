...
"""
==========================================================
  RONEER_ DONOR COLLECTOR
  
  Обновлено: Используем DonationAlerts API для виджетов 
  (со скрытым JSONP API), что позволяет получать реальные
  данные без запуска headless браузера и без OAuth токена.
==========================================================
"""
...
def fetch_widget_data(key, url):
    ...
    try:
        ...
        # Ответ приходит в формате JSONP: ({"data": [...]})
        ...
        for item in items:
            ...
            if name:
                # Извлекаем целую часть суммы для проверки на gold (>= 1000)
