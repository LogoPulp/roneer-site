...
function runPreloaderSequence() {
    ...
    const tick = () => {
        ...
        // Шаги немного случайные по времени, как реальная загрузка
        const delay = current < phases.length ? 300 + Math.random() * 400 : 400;
        setTimeout(tick, delay);
    };

    // Стартуем после того, как буквы «упали» (0.6s задержки CSS)
    setTimeout(tick, 700);
}
...
function setupEasterEgg() {
    ...
    const soundBonk = new Audio('assets/Burp1.mp3'); // Используем то, что есть
    ...
    avatar.addEventListener('click', () => {
        ...
        // Ачивки (логика вынесена в achievements.js, здесь только вызовы)
